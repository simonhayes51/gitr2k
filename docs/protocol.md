...
| `RETRARENALIST` | client→server | country name | `\r\n` terminated, no envelope | `RETRARENALIST International\r\n` | gitr2k.exe requesting the actual per-arena details for one country, sent automatically right after a working `RETRCOUNTRIES` response - not previously known at all, not even as a compiled string constant we'd noticed | **Experiment #1 REJECTED (2026-07-23):** `ARENA <name> <player_count> <max_players>\r\n` + bare `ENDARENALIST\r\n` produced "an error has occured." (RETRCOUNTRIES's own `COUNTRY` line kept working fine throughout, confirming the fault is isolated to this response). **Experiment #2 now live:** bare `ENDARENALIST\r\n` alone (empty list), same isolation technique that fixed `RETRCOUNTRIES`. | CONFIRMED (runtime) for the request; response format still being isolated |
