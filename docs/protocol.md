...
| `RETRCOUNTRIES` | client→server | *(none - sent bare)* | `\r\n` terminated, no envelope | `RETRCOUNTRIES\r\n` | gitr2k.exe requesting the country list for the "Select Arena" tree, sent on the same connection right after a `METAARENALIST` response | **Experiment #2 REJECTED (2026-07-23):** `COUNTRY <name>\r\n` + `ARENA <name> <players> <max>\r\n` + bare `ENDCOUNTRYLIST\r\n` was received and processed by the client, but produced a real error status ("an error has occured.") instead of populating the tree - a genuine negative result, not silence. **Experiment #3 now live:** bare `ENDCOUNTRYLIST\r\n` alone (no COUNTRY/ARENA lines), to isolate whether the per-item line format or something more basic is at fault. | CONFIRMED (runtime) for the request; response format still being isolated experimentally |
