...
| `RETRARENALIST` | client→server | country name | `\r\n` terminated, no envelope | `RETRARENALIST International\r\n` | gitr2k.exe requesting the actual per-arena details for one country, sent automatically right after a working `RETRCOUNTRIES` response - not previously known at all, not even as a compiled string constant we'd noticed | **Six straight rejections** of every `ARENA`-prefixed shape tried (name-only, +1, +2, +host+port, +country). **Experiment #2 CONFIRMED WORKING:** empty list (bare `ENDARENALIST\r\n`). **Returned to static analysis** (2026-07-23): found a previously-missed distinct token, `"GETARENA "` (9 bytes, trailing space baked in - same pattern as confirmed `METAMSG`), separate from the 5-byte `ARENA` constant every guess so far had used. **Experiment #7 now live:** `GETARENA <name> <player_count> <max_players>\r\n` as the line prefix instead. | CONFIRMED (runtime) for the request and the empty-list shape; per-item line prefix now testing a newly-found alternate token |
