...
| `OWNER` | arena.exe | server→client (assumed), candidate `MCC` acknowledgment token | CONFIRMED (disassembly) — found near `GMCC.OnArenaOwner`; its trivial getter function is actively referenced (unlike `GRANTED`) | UNKNOWN — active experiment, see §5 |
...
| `MCC` | client→server | arena name, arena password | space, `\r\n` terminated | `CLAUTH Test none MCC 0\r\n` | arena.exe registering itself with the meta server | **ACTIVE EXPERIMENT (2026-07-23):** `OWNER <arena name>\r\n`, e.g. `OWNER Test\r\n` - informed by the `GMCC.OnArenaOwner` event and the `OWNER` string constant found near it (see §2, §4), but not confirmed by client reaction yet. arena.exe's "Start" button has stayed disabled and its activity log empty in every test so far with no response sent; this is the first attempt. | CONFIRMED (runtime) for the request; response is a disclosed guess |
