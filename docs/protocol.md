...
| `RETRARENALIST` | client→server | country name | `\r\n` terminated, no envelope | `RETRARENALIST International\r\n` | gitr2k.exe requesting the actual per-arena details for one country, sent automatically right after a working `RETRCOUNTRIES` response - not previously known at all, not even as a compiled string constant we'd noticed | **ACTIVE EXPERIMENT (2026-07-23, unconfirmed):** server replies `ARENA <name> <player_count> <max_players>\r\n` per arena, then bare `ENDARENALIST\r\n`, reusing both already-known tokens. Country filtering not implemented yet - every registered arena is returned regardless of which country was requested. | CONFIRMED (runtime) for the request; response is a disclosed guess |
