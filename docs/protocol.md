...
| `BOGUS` | gitr2k.exe | client→server, sent bare | CONFIRMED (runtime) — sent repeatedly, ~every 40s, while a request sits unanswered (confirmed periodic, not one-shot) | UNKNOWN — active experiment: echoing it back verbatim, to test whether acknowledging it unblocks whatever the client is actually waiting on |
