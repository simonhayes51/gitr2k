...
- If the prefix does **not** exactly match `"MOVESDB"`, the **entire line** (not just a remainder) is handed to a further virtual method (`VMT+0x9c`) not disassembled this pass - so what the real, *accepted* response looks like remains completely UNKNOWN. Finding out would require locating the concrete class VMT behind that dynamic dispatch and disassembling whatever function actually lives at that slot - a meaningfully bigger task than anything solved so far in this project (it needs VMT reconstruction, not just a linear disassembly pass). See `commands/reqmoves.py` for the current (reverted-to-silent) state.
...
| `REQMOVES` | client→server | *(none - sent bare)* | `\r\n` terminated, no envelope | `REQMOVES\r\n` | arena.exe requesting some kind of moves/moveset database, sent bare right after `SETINFO`; matches the "retrieving moves database" UI status text | **Experiment #1 REJECTED (2026-07-23):** bare `MOVESDB\r\n`, informed by an initial (incorrect) reading of the `TMovesDBCommand` disassembly. Real client reaction: an explicit `"Could not connect to main GITR server... Exception: invalid response"` error - worse than the prior silent "stuck" state. Corrected disassembly (§4) confirms this was the *expected* outcome of an exact match to `"MOVESDB"` - the check rejects that exact literal, it doesn't require it. Reverted to no response while a real second guess is designed; what the accepted response looks like is still completely UNKNOWN. | CONFIRMED (runtime) for the request; first response guess CONFIRMED WRONG, no replacement guess deployed yet |
...
12. **What is the real `REQMOVES` response format, and does arena.exe's "Start" button ever enable?** Framing bug fixed (2026-07-23) - `REQMOVES` now correctly reaches `commands/reqmoves.py` and is logged. Static analysis found `TMovesDBCommand` in arena.exe; **experiment #1 (bare `MOVESDB\r\n`) was tried and confirmed REJECTED** by real client reaction (explicit `"Exception: invalid response"`) - a corrected re-read of the same disassembly (§4) shows this was the expected outcome of an exact match to `"MOVESDB"`, not a missing one. Reverted to no response. Still completely open: what response format IS accepted? Answering this needs disassembling the actual implementation behind the `VMT+0x9c` virtual call, which requires locating the relevant class's VMT in the data section - a bigger task than any single-function trace done so far in this project. Worth deciding whether to invest in that next, or deprioritize arena-hosting/`REQMOVES` in favor of resuming `RETRARENALIST` (also still open, also paused).
