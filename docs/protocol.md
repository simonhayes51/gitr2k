...
| `RETRARENALIST` | client→server | country name | `\r\n` terminated, no envelope | `RETRARENALIST International\r\n` | gitr2k.exe requesting the actual per-arena details for one country, sent automatically right after a working `RETRCOUNTRIES` response - not previously known at all, not even as a compiled string constant we'd noticed | **Five straight rejections (#1, #3, #4, #5, and the original #1 shape):** name-only, name+1 field, name+2 fields, and name+host+port all produce "an error has occured." - no longer converging as a field-count problem. **Experiment #2 CONFIRMED WORKING:** empty list (bare `ENDARENALIST\r\n`) - no error. **Experiment #6 now live:** new hypothesis - `ARENA <country> <name> <player_count> <max_players>\r\n`, restating the country per-arena since this response is a flat list. If this also fails, planned next step is returning to static analysis rather than continuing to guess field combinations. | CONFIRMED (runtime) for the request and the empty-list shape; `ARENA` line format still unconfirmed after 5 attempts |
