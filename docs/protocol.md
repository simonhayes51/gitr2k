...
| `RETRCOUNTRIES` | client→server | *(none - sent bare)* | `\r\n` terminated, no envelope | `RETRCOUNTRIES\r\n` | gitr2k.exe requesting the country list for the "Select Arena" tree, sent on the same connection right after a `METAARENALIST` response | **Experiment #2 REJECTED:** `COUNTRY`/`ARENA` per-item lines produced a real error status ("an error has occured."). **Experiment #3 CONFIRMED WORKING (2026-07-23):** bare `ENDCOUNTRYLIST\r\n` alone (empty list) - client showed its normal "No arenas online. Start own arena server?" prompt, no error. This isolates the fault to the per-item `COUNTRY`/`ARENA` line format specifically, not the basic framing/terminator. **Experiment #4 now live:** a single bare `COUNTRY <name>\r\n` line with zero `ARENA` lines, to narrow further. | CONFIRMED (runtime) for the request and for the empty-list response shape; per-item line format still being isolated |
...
9. **Is there a separate per-country terminator, or does `ENDCOUNTRYLIST` alone end everything?** `ENDARENALIST` (used to terminate the `METAARENALIST` response) and `ENDCOUNTRYLIST` (assumed to terminate the `RETRCOUNTRIES` response) are two different tokens - UNKNOWN whether `ENDARENALIST` also has a role nested inside a multi-country `RETRCOUNTRIES` response (e.g. one per country's arena sublist) that our current experiment doesn't use.
