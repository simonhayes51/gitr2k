...
| `REQMOVES` | arena.exe | client→server, sent bare immediately after `SETINFO`, no arguments | CONFIRMED (runtime, 2026-07-23) — new token, not previously known | UNKNOWN — matches the "retrieving moves database" UI text; response format not yet attempted, see §5/§6 |
...
| `REQMOVES` | client→server | *(none - sent bare)* | `\r\n` terminated, no envelope | `REQMOVES\r\n` | arena.exe requesting some kind of moves/moveset database, sent bare right after `SETINFO`; matches the "retrieving moves database" UI status text | **UNKNOWN.** No response attempted yet - deliberately observing only (see `commands/reqmoves.py`) to gather evidence (does it retry if unanswered? does the UI ever move past "retrieving moves database"?) before guessing, per the project's isolate-one-variable methodology. | CONFIRMED (runtime) for the request; response completely unknown |
...
12. **What is the real `REQMOVES` response format, and does arena.exe's "Start" button ever enable?** New (2026-07-23), from the `MCC`/`OWNER` breakthrough session. Server currently only observes/logs `REQMOVES` (see `commands/reqmoves.py`) - no response has been attempted yet. Next real test should report: does `REQMOVES` repeat/retry like `BOGUS` did? Does the UI ever move past "retrieving moves database"? Does "Start" ever become clickable without a `REQMOVES` response? **Note:** the very first real attempt to observe `REQMOVES` revealed it had been silently dropped by a framing bug (see the "Bug found and fixed" note in §3) - this question is still open because we've never actually seen the server successfully log a `REQMOVES` request yet, only reconstructed that it must have arrived from the client's subsequent behavior. The framing fix should make the next test the first one to actually capture it.
