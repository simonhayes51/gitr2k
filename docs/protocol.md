...
| `RETRARENALIST` | client→server | country name | `\r\n` terminated, no envelope | `RETRARENALIST International\r\n` | gitr2k.exe requesting the actual per-arena details for one country, sent automatically right after a working `RETRCOUNTRIES` response - not previously known at all, not even as a compiled string constant we'd noticed | **Experiments #1 and #3 REJECTED:** both `ARENA <name> <player_count> <max_players>\r\n` and bare `ARENA <name>\r\n` (zero extra fields) error - breaking the analogy with `COUNTRY`, where name-alone failed but name+one-field succeeded. **Experiment #2 CONFIRMED WORKING:** empty list (bare `ENDARENALIST\r\n`) - no error, so the fault is specifically in the `ARENA` line, just not simply "missing trailing fields" the same way. **Experiment #4 now live:** `ARENA <name> <player_count>\r\n` (single field, no max_players). | CONFIRMED (runtime) for the request and the empty-list shape; `ARENA` line format still being isolated |
