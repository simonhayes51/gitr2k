...
| `RETRARENALIST` | client→server | country name | `\r\n` terminated, no envelope | `RETRARENALIST International\r\n` | gitr2k.exe requesting the actual per-arena details for one country, sent automatically right after a working `RETRCOUNTRIES` response - not previously known at all, not even as a compiled string constant we'd noticed | **Experiments #1, #3, #4 REJECTED:** all three field-count variants of `ARENA <name> [...]` (zero, one, or two trailing numeric fields) error - unlike `COUNTRY`, this isn't a simple missing-field problem. **Experiment #2 CONFIRMED WORKING:** empty list (bare `ENDARENALIST\r\n`) - no error. **Experiment #5 now live:** pivoting to a host/port hypothesis - `ARENA <name> <host_ip> <port>\r\n` - since an arena entry needs connection info to be useful, unlike a country label. | CONFIRMED (runtime) for the request and the empty-list shape; `ARENA` line format still being isolated |
