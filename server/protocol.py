...
KNOWN_TOKENS: Dict[str, TokenInfo] = {
    ...
    "MOVESDB":        TokenInfo("MOVESDB", "arena.exe", CONFIRMED_DISASSEMBLY,
                                 ...
                                 "protocol-related class, one of at least three in a chained family "
                                 "(Moves -> Actions -> ...). CORRECTED (runtime+disassembly "
                                 "cross-validated, 2026-07-23): the response must NOT be an exact match "
                                 "to the literal 'MOVESDB' (first 7 chars) - sending exactly that "
                                 "triggers arena.exe's 'invalid response' exception, confirmed both by "
                                 "re-reading the _LStrCmp-based equality check correctly and by the real "
                                 "client's reaction to the (now known wrong) first experiment. VMT "
                                 "reconstruction (2026-07-23, second pass) confirmed the 'success' "
                                 "branch chains directly into a REQACTIONS/ACTIONSDB request using the "
                                 "identical code shape - so this isn't a single request/response, it's "
                                 "one link in a multi-stage startup chain. What content the 'success' "
                                 "path actually expects is still UNKNOWN - would need tracing the "
                                 "object at Self+0xC4's own class. See commands/reqmoves.py."),
    "ACTIONSDB":      TokenInfo("ACTIONSDB", "arena.exe", CONFIRMED_DISASSEMBLY,
                                 "Found (2026-07-23, VMT reconstruction pass) as a second sibling "
                                 "command class in the same family as MOVESDB/SWEARDB, reached directly "
                                 "from TMovesDBCommand's 'success' branch (i.e. this is the NEXT stage "
                                 "in arena.exe's startup chain after moves, not an independent request). "
                                 "Its request token is 'REQACTIONS' (10 chars); its handler has the "
                                 "identical shape as TMovesDBCommand's - reads one line, rejects an "
                                 "exact match to the literal 'ACTIONSDB' (9 chars) via the same 'invalid "
                                 "response' error, otherwise chains onward again. Not yet observed on "
                                 "the wire (arena.exe never reaches this stage while REQMOVES itself is "
                                 "unanswered). Previously grouped (wrongly, like MOVESDB) with unrelated "
                                 "scripting keywords."),
    "REQACTIONS":     TokenInfo("REQACTIONS", "arena.exe", CONFIRMED_DISASSEMBLY,
                                 "Found (2026-07-23, VMT reconstruction pass) as the request token sent "
                                 "by the ACTIONSDB stage of the same command chain as REQMOVES/MOVESDB. "
                                 "Not yet observed on the wire - see ACTIONSDB."),
